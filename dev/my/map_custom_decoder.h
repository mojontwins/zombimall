...
#define RLE_FORMAT 62
	#asm
		...
		._advance_worm
			...
			// -- INSERTO
            // gpit está en bc, en realidad en c.

            // Si gpit < paint_y1 OR gpit >= paint_y2 => _t = 0
            ...
            ld  a, c                            ; A = gpit
            cp  b                               ; A comp B
            jr  nc, _advance_worm_zero          ; Si A < B (gpit < paint_y2) -> sigue
                                                ; Si A >= B (gpit >= paint_y2) -> GOTO advance_worm_zero
            ...
            ld  a, c                            ; A = gpit
            cp  b                               ; A comp B
            jr  nc, _advance_worm_draw          ; Si A >= B (gpit >= paint_y1) -> GOTO advance_worm_draw
                                                ; Si A < B (gpiy < paint_y1) -> sigue (advance_worm_zero)
          
        ._advance_worm_zero
            xor a
            ld  (__t), a                        ; ponemos un 0
